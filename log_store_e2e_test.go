...
func TestGetLogsTestSuiteE2E(t *testing.T) {
	suite.Run(t, new(GetLogsE2ETestSuite))
}

type GetLogsE2ETestSuite struct {
	suite.Suite
	env    TestEnvInfo
	client *Client
}

func (s *GetLogsE2ETestSuite) SetupSuite() {
	testutil.RequireE2E(s.T())
...
func (s *GetLogsE2ETestSuite) TearDownSuite() {

}

func (s *GetLogsE2ETestSuite) TestGetLogsV2() {
...
func (s *GetLogsE2ETestSuite) TestGetLogLinesV2() {
	exists, err := s.client.CheckProjectExist(s.env.ProjectName)
	s.Require().NoError(err)
	s.True(exists)
	t := uint32(time.Now().Unix())
